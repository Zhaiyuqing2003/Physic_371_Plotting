var findWords = function (board, words) {
    ...
    // get the letter count of the board
    const boardLetterCount = {};
    for (let i = 0; i < board.length; i++) {
        for (let j = 0; j < board[i].length; j++) {
            const letter = board[i][j];
            if (boardLetterCount[letter]) {
    ...
    // console.log(boardLetterCount);

    for (let word of words) {
        // console.log("checking word ", word);
        // get the letter count of the word
        ...
        for (let letter in wordLetterCount) {
            if (boardLetterCount[letter] === undefined || wordLetterCount[letter] > boardLetterCount[letter]) {
                canForm = false;
                break;
            }
        }
        ...
        const boardMarked = board.map(row => row.map(() => false));
        outerLoop:
        for (let i = 0; i < board.length; i++) {
            for (let j = 0; j < board[0].length; j++) {
                if (board[i][j] === word[0]) {
                    boardMarked[i][j] = true;
                    // console.log("start from", board[i][j], i, j);
                    if (findWord(board, word.slice(1), i, j, boardMarked)) {
                        result.push(word);
                        break outerLoop;
                    }
...
var findWord = function (board, word, x, y, boardMarked) {
    if (word.length === 0) return true;
    const walkAllowed = [];

    if (x - 1 >= 0 && boardMarked[x - 1][y] === false) {
        walkAllowed.push([x - 1, y]);
    }

    if (x + 1 < board.length && boardMarked[x + 1][y] === false) {
        walkAllowed.push([x + 1, y]);
    }

    if (y - 1 >= 0 && boardMarked[x][y - 1] === false) {
        walkAllowed.push([x, y - 1]);
    }

    if (y + 1 < board[0].length && boardMarked[x][y + 1] === false) {
        walkAllowed.push([x, y + 1]);
    }

    for (let i = 0; i < walkAllowed.length; i++) {
        const [x, y] = walkAllowed[i];
        // console.log("walk to", board[x][y], x, y);
        if (board[x][y] === word[0]) {
            boardMarked[x][y] = true;
            if (findWord(board, word.slice(1), x, y, boardMarked)) return true;
            boardMarked[x][y] = false;
        }
    }
